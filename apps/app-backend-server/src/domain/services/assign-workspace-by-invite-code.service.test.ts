...
beforeEach(() => {
  container.snapshot();
  container.rebind<AssignWorkspaceByInviteCodeService>(INJECT_KEY.AssignWorkspaceByInviteCodeService).to(AssignWorkspaceByInviteCodeService);
});
...
describe('Positive', () => {
  it('If the workspace is assigned successfully', async () => {
    ...
    const workspace = await container.get<AssignWorkspaceByInviteCodeService>(INJECT_KEY.AssignWorkspaceByInviteCodeService).execute({
...
describe('Negative', () => {
  it('If the user is not found', async () => {
    ...
    await expect(hasAsyncThrow(async () => await container.get<AssignWorkspaceByInviteCodeService>(INJECT_KEY.AssignWorkspaceByInviteCodeService).execute({
      userId: mockedUserId.toString(),
      inviteCode: mockedWorkspaceInviteCode.toString(),
