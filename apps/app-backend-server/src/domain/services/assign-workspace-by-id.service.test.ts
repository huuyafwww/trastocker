...
beforeEach(() => {
  container.snapshot();
  container.rebind<AssignWorkspaceByIdService>(INJECT_KEY.AssignWorkspaceByIdService).to(AssignWorkspaceByIdService);
});
...
describe('Positive', () => {
  it('If the workspace is assigned successfully', async () => {
    ...
    const workspace = await container.get<AssignWorkspaceByIdService>(INJECT_KEY.AssignWorkspaceByIdService).execute({
...
describe('Negative', () => {
  it('If the user is not found', async () => {
    ...
    await expect(hasAsyncThrow(async () => await container.get<AssignWorkspaceByIdService>(INJECT_KEY.AssignWorkspaceByIdService).execute({
      userId: mockedUserId.toString(),
      workspaceId: mockedWorkspaceId.toString(),
